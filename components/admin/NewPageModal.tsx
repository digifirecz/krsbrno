...
export default function NewPageModal({ open, onClose, onCreate }: NewPageModalProps) {
  ...
  const handleSubmit = async () => {
    if (!label.trim()) {
      setError('Zadejte název stránky.');
      return;
    }
    setError('');
    setSubmitting(true);
    try {
      ...
      setError(err instanceof Error ? err.message : 'Stránku se nepodařilo vytvořit.');
      setSubmitting(false);
    }
  };
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b pb-3">
          <h3 className="text-lg font-bold font-serif">Nová stránka</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div>
          <label className="block text-xs font-bold text-neutral-600 mb-1">Název stránky</label>
          <input
            type="text"
            value={label}
            ...
            placeholder="Např. Vánoční bohoslužba"
          ...
          <p className="text-xs text-neutral-400 mt-1.5">
            URL adresa stránky se vytvoří automaticky z názvu.
          </p>
        ...
        <div className="flex justify-end space-x-2 pt-2">
          ...
          >
            {submitting ? 'Vytváří se…' : 'Vytvořit stránku'}
          </button>
        </div>
      </div>
    </div>
