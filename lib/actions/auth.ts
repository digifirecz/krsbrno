...
export const login = withAction('auth.login', async (email: string, password: string): Promise<{ ok: boolean; error?: string }> => {
...
export const logout = withAction('auth.logout', async (): Promise<void> => {
  await clearSession();
});

export const getCurrentUser = withAction('auth.getCurrentUser', async () => {
  return getSession();
});

// Vždy tichý úspěch — neprozrazuje, jestli účet existuje.
export const requestPasswordReset = withAction('auth.requestPasswordReset', async (email: string): Promise<void> => {
  const key = email.trim().toLowerCase();
  if (!key) return;
...
export const resetPassword = withAction('auth.resetPassword', async (token: string, newPassword: string): Promise<{ ok: boolean; error?: string }> => {
...
export const adminSetPassword = withAction('auth.adminSetPassword', async (email: string, newPassword: string, actorEmail?: string | null): Promise<{ ok: boolean; error?: string }> => {
...
export const changePassword = withAction('auth.changePassword', async (currentPassword: string, newPassword: string): Promise<{ ok: boolean; error?: string }> => {
  const session = await requireSession();
  return data.changeOwnPassword(session.email, currentPassword, newPassword);
});

export const getMustChangePassword = withAction('auth.getMustChangePassword', async (): Promise<boolean> => {
  const session = await getSession();
